...
public class MagicPacketPlugin {
  ...
  private static native void execImpl (String action, MagicPacket value, JsCallback success, JsCallback failure) /*-{
    var jsSuccess = $entry(function(results) {
      success.@it.mate.onscommons.client.utils.MagicPacketPlugin.JsCallback::execute(Lcom/google/gwt/core/client/JavaScriptObject;)(results);
    });
    var jsFailure = $entry(function(err) {
      failure.@it.mate.onscommons.client.utils.MagicPacketPlugin.JsCallback::execute(Lcom/google/gwt/core/client/JavaScriptObject;)(err);
    });
