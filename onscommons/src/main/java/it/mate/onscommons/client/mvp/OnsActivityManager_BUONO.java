...
public abstract class OnsActivityManager_BUONO extends ActivityManager {
  ...
  public OnsActivityManager_BUONO(OnsActivityMapper mapper, EventBus eventBus) {
  ...
  private void setActiveTemplate(HasToken place) {
    ...
    if (activeTemplate != null) {
      CdvUtils.log("CLEARING TEMPLATE " + activeTemplate.getId());
      ...
      OnsenUi.onPagePushed(new Delegate<NavigatorEvent>() {
        public void execute(NavigatorEvent event) {
          ...
          if (enteringPage != null) {
            ...
            CdvUtils.log("ENTERING PAGE " + CdvUtils.elementToString(enteringPageContext));
//          CdvUtils.callDebugHook(enteringPageContext);
